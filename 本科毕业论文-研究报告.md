...
Initialize x
for each iteration:
    generate K proposals y1,...,yK from Normal(x, s^2)
    compute weights wj ∝ exp(logπ(yj))
    select y* by normalized weights
    draw auxiliary point z around current x
    compute acceptance ratio using selected/backup/current/auxiliary terms
    accept or reject y*
```
...
实验依据 `master` 分支已有记录与实现流程。关键数值来源于项目记录表（证据来源：`README.md`；参数：`n_samples=20000`；随机种子：`42`），并由源码逻辑给出对应计算路径（证据来源：`src/mcmc_optimized.py`）。
...
从统计效率看，K=4 时 IAT 由 8 降至 4，ESS 翻倍；但从时间看，运行成本显著上升。这说明多提案机制在统计效率与计算代价之间存在明确权衡。
...
结果表明，K 增大时 IAT 下降，但在串行实现下时间开销上升更快，因此墙钟时间不呈现加速。该现象可由 `run_speedup_analysis` 的输出逻辑直接对应（证据来源：`src/mcmc_optimized.py`；配置：`n_samples=20000, burn_in=n_samples//4`；随机种子：`42`）。

4.4 与 Monte Carlo 基线比较

基于 `run_baseline_comparison` 的记录（证据来源：`src/mcmc_optimized.py`；参数：`n_samples=50000`；随机种子：`42`），可得到表4-3所示结论。
...
本章表明：MTM 在统计效率方面相对 RWMH 具有优势，但其计算成本并非总能转换为时间加速。该结论与多提案算法在串行环境下的实际表现一致，也为后续并行化与自适应策略研究提供了方向。

第5章 结论与展望

5.1 主要结论

本文基于 `master` 分支实现，围绕“多提案 MCMC 在期权定价中的效率表现”进行了系统分析，主要结论如下：
...
- 关键参数：`S0=100, K=100, T=1, r=0.05, sigma=0.2, n_samples=20000/50000`
- 随机种子：`np.random.seed(42)`（见主程序入口）
